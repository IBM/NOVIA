...
BENCH_DIR=../../benchmarks/simplebr/

${LLVM_ROOT}/bin/opt -load ${PASSES_DIR}libfusionlib.so -renameBBs < ${BENCH_DIR}simplebr.bc  > ${BENCH_DIR}simplebr_rn.bc
${LLVM_ROOT}/bin/opt -load ${PASSES_DIR}libfusionlib.so -mergeBBList -bbs bblist.txt -fmerg --graph_dir imgs < ${BENCH_DIR}simplebr_rn.bc  > out.bc
